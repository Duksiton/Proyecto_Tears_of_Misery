document.addEventListener('DOMContentLoaded', function() {
    const carrito = JSON.parse(localStorage.getItem('carrito')) || []; // Cargar el carrito desde localStorage
    ...
    sizeSelect.addEventListener('change', function() {
        addToCartButton.disabled = this.value === "";
    });

    addToCartButton.addEventListener('click', function() {
        if (sizeSelect.value === "") {
            alert('Por favor, elige una talla antes de agregar el producto al carrito.');
            return;
        }
    ...
    function actualizarCarrito() {
        ...
        itemsMostrar.forEach((item, index) => {
            ...
            row.innerHTML = `
                ...
                <td style="color: #000;">${formatearPrecio(item.price)}</td>
                ...
                <td><button class="btn-comprar" data-index="${index}" style="background-color: #1e7e34; color: white; border: none; padding: 5px 10px; border-radius: 4px; cursor: pointer;">Comprar</button></td>
            `;
        });
